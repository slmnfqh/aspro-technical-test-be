                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @foreach ($projects as $project)
                            <tr
                                class="border-b dark:border-gray-700 {{ $project->deleted_at ? 'bg-gray-300 dark:bg-gray-700' : '' }}">
                                <td class="px-4 py-3 text-black">{{ $loop->iteration }}</td>
                                <th scope="row"
                                    class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    @if ($project->deleted_at)
                                        <del class="text-gray-500">{{ $project->name }}</del>
                                    @else
                                        {{ $project->name }}
                                    @endif
                                </th>
                                <td class="px-4 py-3 text-black">
                                    @if ($project->deleted_at)
                                        <del class="text-gray-500">{{ $project->category->name }}</del>
                                    @else
                                ...
                                <td class="px-4 py-3 text-black">
                                    @if ($project->deleted_at)
                                        <del class="text-gray-500"> {!! Str::limit(strip_tags($project->description), 20) !!}</del>
                                    @else
                                ...
                                <td class="px-4 py-3 flex gap-2 items-center">
                                    @if (!$project->deleted_at)
                                        ...
                                        <x-button.projects.button-update :project="$project"
                                            :categories="$categories"></x-button.projects.button-update>
                                        <form action="/dashboard/projects/{{ $project->slug }}" method="post"
                                            class="inline m-0">
                                            @method('delete')
                                            @csrf
                                            <button
                                    ...
                                    @endif
                                    @if ($project->deleted_at)
                                        <form action="{{ route('dashboard.projects.restore', $project->id) }}"
